...
void adicionaAresta(ListaAdj* lista, int destino, int peso) {
    for (int i = 0; i < lista->tamanho; i++) {
        if (lista->arestas[i].destino == destino) {
            if (lista->arestas[i].peso > peso) {
                lista->arestas[i].peso = peso;
            }
            return;
        }
    }
...
void insereHeap(MinHeap* heap, int vertice, int distancia) {
    if (heap->tamanho == heap->capacidade) {
        heap->capacidade *= 2;
        heap->elementos = (ElementoHeap*)realloc(heap->elementos, heap->capacidade * sizeof(ElementoHeap));
    }
    int i = heap->tamanho++;
...
int main() {
    ...
    for (int i = 0; i < E; i++) {
        ...
        scanf("%d %d %d", &A, &B, &W);
        if (W < 0) {
            printf("Erro: Arestas de peso negativo não são permitidas.\n");
            return 1;
        }
