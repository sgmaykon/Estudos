/* 
Nome: Maykon dos Santos Gonçalves
Curso: Engenharia de Computação
RA: 821653
*/
/*              ========== DISCLAIMER =============
    Então, aconteceu algo que eu não sei bem explicar. Na minha
    ideia original, eu ia adicionar zeros a direita do número caso
    a qtde de dígitos fosse diferente. Pra isso, ia multiplicar por
    10 o valor , até a qtd de numeros ser igual
    Acontece que pra ter o resultado correto, no fim, eu teria que 
    dividir o resultado por 10*(numr de zeros adicionados)
    então por exemplo, se adicionei 2 dígitos, seria
    (a* expRec(10,n1) + d*(expRec(10,n1/2)) +b)/10**2
    
    Só que, um ";" passou batido por mim, e uma das verificações foi ignorada
                if( (n1 == 1) & (n2 ==1) );
                    return x*y;
    Se o tamanho das duas for de apenas 1 dígito, retorna uma multiplicação
    comum. Nesse caso, a verificação ta sendo ignorada, e ta 
    retornando direto x*y. Por algum motivo, isso funciona, e posso
    usar o resultado final do karatsuba sem dividir por nada.
    Dei sorte, acho que daria muito mais trabalho se eu tivesse feito
    do jeito que planejei originalmente XD.
...
double karatsuba(double x, double y){ 
        ...
        int n2 = DigitCount(y);
        if( (n1 == 1) & (n2 ==1) ) //Olhar DISCLAIMER
       		return x*y;
        int diff = n1-n2;   //Se os tam são diferentes, preciso saber quem é maior pra adicionar 0.
        ...
        unsigned long int a = karatsuba(xe, ye); // unsigned long int pra ter mais espaço
        unsigned long int b = karatsuba(xr,yr);
        unsigned long int c = karatsuba(xe + xr, ye + yr);
        unsigned long int d = c-a-b;
        double result = (a* expRec(10,n1) + d*(expRec(10,n1/2)) +b)/(expRec(10,diff));
        return result;
